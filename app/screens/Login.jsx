...
const Login = ({ navigation }) => {
	...
	const errorAlert = (
		<View style={tailwind("bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded my-5")}>
			...
			<TouchableOpacity style={tailwind("absolute flex justify-center top-0 bottom-0 right-0 px-4")} onPress={() => setError("")}>
					...
					{({ errors, handleSubmit, handleChange, handleBlur, values }) => (
						<View style={tailwind("md:w-1/2")}>
							...
							<View style={tailwind("mt-3")}>
								<Button style={tailwind("font-bold px-4 rounded h-full")} color={"#9400D3"} onPress={handleSubmit} title='Login' />
							</View>
							<View style={tailwind("py-4 flex items-center text-center")}>
								<Text onPress={() => navigation.navigate("SignUp")}>
									No account?&nbsp;<Text style={tailwind("font-bold underline")}>Register here</Text>{" "}
								</Text>
							</View>
						</View>
					)}
